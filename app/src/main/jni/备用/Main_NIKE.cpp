...
#define ProcessEvent_Offset 0x716BE7C
...
if (大厅) {
写入Q类(readValueL(UE4+0x377F8)+0x8,256);
写入Q类(readValueL(UE4+0x377F8)+0x84,256);
写入Q类(readValueL(UE4+0x377F8)+0x34,256);
写入Q类(readValueL(UE4+0x377F8)+0x14,256);
写入Q类(readValueL(UE4+0x377F8)+0xD4,256);
写入Q类(readValueL(UE4+0x377F8)+0xFC,256);
写入Q类(readValueL(UE4+0x37828)+0x8,256);
写入Q类(readValueL(UE4+0x37828)+0x84,256);
写入Q类(readValueL(UE4+0x37828)+0x34,256);
写入Q类(readValueL(UE4+0x37828)+0x14,256);
写入Q类(readValueL(UE4+0x37828)+0xD4,256);
写入Q类(readValueL(UE4+0x37828)+0xFC,256);
写入Q类(readValueL(UE4+0x37830)+0xF4,256);
写入Q类(readValueL(UE4+0x37860)+0xF4,256);
写入Q类(readValueL(UE4+0x36EB28)+0xEC,256);
写入Q类(readValueL(UE4+0x392228)+0xBC,256);
写入Q类(readValueL(UE4+0x3A8180)+0x4,256);
写入Q类(readValueL(UE4+0x3E8E70)+0x34,256);
写入Q类(readValueL(UE4+0x6E88E8)+0x34,256);
写入Q类(readValueL(UE4+0x6E88E8)+0x14,256);
写入Q类(readValueL(UE4+0x6E88E8)+0xFC,256);
写入Q类(readValueL(UE4+0x6E8918)+0x34,256);
写入Q类(readValueL(UE4+0x6E8918)+0x14,256);
写入Q类(readValueL(UE4+0x6E8918)+0xFC,256);

}
if (防封) {
写入D类(TDataMaster+0x3E8644, 67109633);//通杀闪退
PATCH_LIB("libUE4.so","0x626FF6C","1F 20 03 D5");
 PATCH_LIB("libUE4.so","0x63F5A60","C0 03 5F D6");
HOOK_LIB("libanort.so","0x18AE7C","00 00 80 D2 C0 03 5F D6");
HOOK_LIB("libanort.so","0x1A76C4","00 00 80 D2 C0 03 5F D6");
MemoryPatch::createWithHex("libanogs.so",0x359D8,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x359E0,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x359E4,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x359E8,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35AAC,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35AEC,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35CD8,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35D84,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35D88,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35D90,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35DD4,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35E60,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35E6C,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35E98,"h 80 42 00 00").Modify();
MemoryPatch::createWithHex("libanogs.so",0x35E9C,"h 80 42 00 00").Modify();
...
if (信息) {
        ...
        if (信息) {
            std::string s;
                                ...
                                draw->AddText(nullptr, 18.f, ImVec2(HeadPosSC.x - 28, HeadPosSC.y - 36.0f), IM_COL32(255, 255, 255, 255), s.c_str());
        }
        }
}}
if (骨灰盒) {
...
ImGui::GetForegroundDrawList()->AddText(NULL,35,{(glWidth / 2) - (textSize.x / 2),41},ImColor(255,000,000,255),安全.c_str());//安全文字(红色 全色)
...
安全 += "悬浮已显示";
auto textSize = ImGui::CalcTextSize2(安全.c_str(), 0, 35);
ImGui::GetForegroundDrawList()->AddText(NULL,35,{(glWidth / 2) - (textSize.x / 2),41},ImColor(255,255,255,255),安全.c_str());//安全文字(红色 全色)
...
g_LocalController = localController;
g_LocalPlayer = localPlayer;
std::string s = "Boxs " + std::to_string((int)箱子数量) + " : Players " + std::to_string((int)totalEnemies) + " : Bots " + std::to_string((int)totalBots) + " : Dogs " + std::to_string((int)狗子数量);
ImVec2 textSize = ImGui::CalcTextSize2(s.c_str(), 0, ((float)density / 16.5f));
ImVec2 vStart = {glWidth / 2 - 100, 70};
ImVec2 vEnd = {glWidth / 2 + 100, 100};  
...
draw->AddText(nullptr, ((float)density / 16.5f), textPos, IM_COL32(255, 255, 255, 255), s.c_str());
}}}
...
EGLBoolean _eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    //        style->WindowRounding = 30.0f;  
     //       style->FrameRounding = 3.0f;  			
        ...
        //背景
        style->Colors[ImGuiCol_WindowBg] = ImColor(217, 217, 217,255); 
      //文字颜色    
        style->Colors[ImGuiCol_Text]                   = ImVec4(0, 0, 0, 255);    
       //边框
       style->Colors[ImGuiCol_Border] = ImColor(100, 100, 100);       
        // 子菜单
         style->Colors[ImGuiCol_ChildBg]                           = ImColor(255, 255, 255);
           ...
           //  按钮颜色  
             ...
             //下拉 
               style->Colors[ImGuiCol_PopupBg] = ImColor(215, 0, 0, 255); 
     //   style.ScaleAllSizes(std::max(2.5f, density / 220.0f));
      style->Colors[ImGuiCol_SliderGrab] = ImColor(217, 217, 217); // 滚动条小方块颜
      style->Colors[ImGuiCol_Header]                = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_HeaderActive]          = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_HeaderHovered]         = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ScrollbarBg]           = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ScrollbarGrab]         = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ScrollbarGrabHovered]  = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ScrollbarGrabActive]   = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ResizeGrip]            = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ResizeGripHovered]     = ImColor(217, 217, 217);
      style->Colors[ImGuiCol_ResizeGripActive]      = ImColor(217, 217, 217);    
    style->Colors[ImGuiCol_SliderGrabActive] = ImColor(217, 217, 217); 
   
             style->Colors[ImGuiCol_FrameBg] = ImColor(220, 220, 220, 215); // 框架背景的默认颜色（例如输入框）
      style->Colors[ImGuiCol_FrameBgActive] = ImColor(220, 220, 220, 215); // 框架背景在激活状态下的颜色
      style->Colors[ImGuiCol_FrameBgHovered] = ImColor(220, 220, 220,215); // 鼠标悬停在框架背景上时的颜色
      style->Colors[ImGuiCol_CheckMark]             = ImColor(0, 255, 255); //按钮对勾 深色
...
ImGui::TextColored(ImColor(255,0,0,225), "请验证你的卡密\n如果闪退请关闭vpn再登陆");			    


                if (!isLogin) {
            ImGui::Text("请输入卡密! (提前复制卡密)");
            ...
            if (ImGui::Button("输入卡密", ImVec2(ImGui::GetContentRegionAvailWidth(), 0))) {
                auto key = getClipboardText();
                strncpy(s, key.c_str(), sizeof s);
            }
 
              static std::string err;
            if (ImGui::Button("登录入", ImVec2(ImGui::GetContentRegionAvailWidth(), 0))) {
                err = Login(s);
                if (err == "OK") {
...
if (音量键){
副菜单 += 13.5f;//调节开启的速度
if (副菜单 > 530.f)//如果到达此数值则固定
{
副菜单 = 530.f;//固定定义值
}
}else{
副菜单 -= 15.5f;//调节关闭的速度
if (副菜单 < 0.0f)//如果减少到此数值则固定
{
副菜单 = 0.0f;//固定定义值
}}
...
ImGui::SetNextWindowSize(ImVec2(740, openx));
if(音量键 || 悬浮窗){
if (ImGui::Begin(" Qing ", 0, ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar )) {
...
{
ImGui::Image(Logo.textureId, {90, 90});
ImGui::SameLine();
ImGui::TextColored(ImColor(255,0,0,225), "浮梦");
ImGui::TableNextColumn();
if (ImGui::Button("主页",ImVec2(130, 60)))//200
{
菜单 = true;
菜单1 = false;
菜单2 = false;
...
if (ImGui::Button("绘制",ImVec2(130, 60)))//260
{
菜单 = false;
菜单1 = true;
菜单2 = false;
...
if (ImGui::Button("自瞄",ImVec2(130, 60)))//320
{
菜单 = false;
菜单1 = false;
菜单2 = true;
...
/*if (ImGui::Button("其他绘制",ImVec2(130, 60)))//320
{
菜单 = false;
菜单1 = false;
菜单2 = false;
...
if (ImGui::Button("美化",ImVec2(130, 60)))//320
{
菜单 = false;
菜单1 = false;
菜单2 = false;
...
if (ImGui::Button("设置",ImVec2(130, 60)))//320
{
菜单 = false;
菜单1 = false;
菜单2 = false;
...
{
if (ImGui::Button("重启程序",{-1,75})){
重启游戏();
}
if (ImGui::Button("加入频道",{-1,75})){
跳转浏览器("https://t.me/hdbyydsY");
}
static int e = -1;
...
ImGui::Checkbox("天气下雨", &下雨);
ImGui::SameLine();
ImGui::Checkbox("天气下雪", &下雪);
ImGui::SameLine();
ImGui::Checkbox("天气晴天", &晴天);
ImGui::EndTabItem();   
} 
}  
if(菜单1){
...
if (ImGui::BeginChild("子菜单0", ImVec2(0, 0), true, ImGuiWindowFlags_None));
{
ImGui::TextColored(ImColor(255,0,0,225), "(CN) 欢迎使用咸鱼梦想家 (EN) Welcome to use ");
ImGui::Spacing();
...
ImVec2 basicWindowPos = ImVec2(aimbotWindowPos.x + aimbotWindowSize.x + 15.04, aimbotWindowPos.y - 9.62); 
ImVec2 basicWindowSize = ImVec2(ImGui::GetWindowWidth() - aimbotWindowSize.x - 120.00, ImGui::GetWindowHeight());
style.WindowRounding = 0.0f; 
const ImVec2 minWindowSize = ImVec2(367.42, 副菜单);
...
ImGui::Begin("配置", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove); 
   
if (ImGui::BeginChild("子菜单0", ImVec2(0, 0), true, ImGuiWindowFlags_None));
{
ImGui::TextColored(ImColor(255,0,0,225), "(CN) 物资绘制");
ImGui::Spacing();    
//ImGui::TextColored(ImColor(蓝色), "(CN) 绘制菜单  (EN) ESP Menu");
ImGui::Checkbox("步枪", &步枪);
ImGui::SameLine();
ImGui::Checkbox("冲锋", &冲锋枪);
ImGui::SameLine();
...
ImGui::SameLine();
ImGui::Checkbox("散弹", &散弹枪);
ImGui::SameLine();
ImGui::Checkbox("狙击", &狙击枪);
...
{
/*
ImGui::TextColored(ImColor(255,0,0,225), "(CN) 概率子追(仿内核)");
ImGui::Spacing();
static int 废物 = 0;
...
if (ImGui::BeginChild("子菜单0", ImVec2(0, 0), true, ImGuiWindowFlags_None));
{
         ImGui::TextColored(ImColor(255,0,0,225), "(CN) SDK功能");
ImGui::Spacing();    
ImGui::Checkbox("广角", &上帝视角);
ImGui::SliderFloat("视角调节", &视角,100.0f, 200.0f);
ImGui::Checkbox("马可", &马可);
ImGui::SliderFloat("旋转速度", &DancerValue, 0.0f, 50.0f);
ImGui::Checkbox("枪械大小", &大枪);
ImGui::SliderFloat("大小调节", &枪械大小, 0.0f, 5.0f);
ImGui::Checkbox("除雾", &除雾);
ImGui::SameLine();
ImGui::Checkbox("聚点", &据点);
ImGui::SameLine();
ImGui::Checkbox("内透", &WALLHACK);
ImGui::SameLine();
ImGui::Checkbox("除草", &除草);
}}
if(菜单3){
...
if (ImGui::Combo("自选衣服", &style_sn, "果冻 \0金尊法老\0丁香公主\0齐天大圣\0娇俏仙灵\0蓝色木乃伊\0曙光芙洛拉\0丧尸美化\0")) {
switch (style_sn) 
{
case 0:
衣服数值 = 1406971;
...
if (ImGui::Combo("自选手持", &style_fox, "冰霜核心 \0死噶血魂魔\0QBZ瑰绮灵姬\0星云力量\0魔力结晶\0龙吟妹控\0")) {
switch (style_fox) 
{
case 0:
枪械值 = 1101004046;
...
ImGui::Checkbox("飞行器美化", &飞行器美化);
if (ImGui::Combo("自选飞行器", &style_fxq, "奇趣飞行器 \0赛年飞行器\0极寒飞霜飞行器\0奇幻爱意飞行器\0不朽穿梭者飞行器\0黄金飞翼飞行器\0")) {
switch (style_fxq) 
{
case 0:
飞行器 = 1401545;
...
ImGuiViewportP* viewport = (ImGuiViewportP*)ImGui::GetMainViewport();
ImRect viewport_rect = viewport->GetMainRect();
ImGui::TextColored(ImColor(255,0,0,225), "(CN) 美化如果没效果请下载资源");
ImGui::Checkbox("头盔美化", &头盔美化);			
if (ImGui::Combo("自选背包", &style_cd, "愚人小丑背包 \0小猫背包\0血鸦背包\0法老背包\0哥斯拉\0隐藏背包\0")) {
switch (style_cd) 
{
case 0:
背包数值 = 1501003051;
...
ImGui::Checkbox("背包美化", &背包美化);
if (ImGui::Combo("自选头盔", &style_bb, "胖达头盔 \0冰头头盔\0星云捕猎头盔\0愤怒胖红头盔\0御灵师头盔\0隐藏头盔\0")) {
switch (style_bb) 
{
case 0:
头盔数值 = 1502003027;
...
ImGui::Checkbox("特效美化", &打击特效);
if (ImGui::Combo("自选特效", &少年无敌, "机魂忍神 \0庇护之潮\0弃誓冰灵\0掠空之翼\0极尊音律\0")) {
switch (少年无敌) 
{
case 0:
特效数值 = 1101004218;
...
__int64 __fastcall (*osub_166814)(__int64 a1, unsigned __int8 *a2, size_t a3);
...
__int64 __fastcall (*osub_166C24)(__int64 a1);
...
return osub_166C24(a1);
}

int (*osub_1B5688)(int a1, const char *a2);
int sub_1B5688(int a1, const char *a2) {   
...
__int64 (*sub_166CD8)(__int64 a1, const char *a2);
__int64 __fastcall hsub_166CD8(__int64 a1, const char *a2) {
if (
...
__int64 __fastcall (*osub_103790)(__int64 a1, _BYTE *a2, _BYTE *a3, __int64 a4, __int64 a5);
__int64 __fastcall hsub_103790(__int64 a1, _BYTE *a2, _BYTE *a3, __int64 a4, __int64 a5)
{
while(true)
{
sleep(999999);
}
return hsub_103790(a1,a2,a3,a4,a5);
}
...
__int64 __fastcall (*osub_3FD5A4)(const char *a1, unsigned int a2);
__int64 __fastcall sub_3FD5A4(const char *a1, unsigned int a2) {
if (a2, ("start")) {
return 0LL;
}
while (true) {
...
return osub_3FD5A4(a1, a2);
}
__int64 __fastcall (*osub_4021C8)(const char *a1, unsigned int a2);
__int64 __fastcall sub_4021C8(const char *a1, unsigned int a2) {
if (a2, ("start")) {
return 0LL;
}
while (true) {
...
return osub_4021C8(a1, a2);
}
int (*osub_1EDD9C)(int64_t a1, const char *a2);
int __fastcall sub_1EDD9C(int64_t a1, const char *a2) {
if (strstr(a2, "egl"))//内部渲染检查
{return 0;}
return osub_1EDD9C(a1, a2);
}
int ret0()
{
return 0;
}
void __fastcall (*osub_171768)(__int64 a1, __int64 a2, __int64 a3, __int64 a4);
void __fastcall hsub_171768(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
...
while(true)
{
 ...
 return hsub_171768(a1,a2,a3,a4);
}
...
PATCH_LIB("libanogs.so","0x248A8C","00 00 80 D2 C0 03 5F D6"); // Offline Fix离线修复
PATCH_LIB("libanogs.so","0x248304","00 00 80 D2 C0 03 5F D6"); // Offline Fix离线修复
PATCH_LIB("libanogs.so","0x1F4464","00 00 80 D2 C0 03 5F D6"); // Offline Fix离线修复
PATCH_LIB("libanogs.so","0x2BD074","00 00 80 D2 C0 03 5F D6"); // Offline Fix离线修复
...
/*
HOOK_LIB("libanogs.so","0x1B5688",sub_1B5688,osub_1B5688);//XVALUES
HOOK_LIB("libanogs.so", "0x173ED4", hsub_173ED4, osub_173ED4); //case 16 (crash fix)
//HOOK_LIB("libanogs.so","0x1EDD9C",sub_1EDD9C,osub_1EDD9C);//防egl追封
//HOOK_LIB_NO_ORIG("libanogs.so", "0xDF3A0", ret0);//kill 杀67防追
/*HOOK_LIB("libanogs.so", "0x166C24", sub_166C24, osub_166C24);(case 35)*/
...
void *HOOK防闪_thread(void *) {
 ...
 PATCH_LIB("libanogs.so","0xDF2FC","C0 03 5F D6");
PATCH_LIB("libanogs.so","0x1229E4","h 00 00 00 01");
    return 0;
}
